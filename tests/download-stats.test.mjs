...
test('기록은 날짜별·파일별로 서버 증가값을 PATCH 하고, 실패해도 던지지 않는다', async () => {
  ...
  assert.equal(calls[0].url, 'https://soc-c-qna-default-rtdb.firebaseio.com/stats/downloads/2026-09-18/' + encodeURIComponent('학습지｜기후·pdf') + '.json');
  assert.equal(calls[0].init.method, 'PATCH'); assert.equal(calls[0].init.keepalive, true);
  assert.deepEqual(JSON.parse(calls[0].init.body), {download: {'.sv': {increment: 1}}});
